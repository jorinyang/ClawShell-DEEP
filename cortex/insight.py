...
class InsightEngine:
    ...
    def __init__(self, node_id: NodeID, event_bus: EventBus):
        ...
        self.event_history: deque[EventMessage] = deque(
    ...
    async def _on_error(self, event: EventMessage):
        ...
        self._error_count[event.source] = (
            self._error_count.get(event.source, 0) + 1
        )
        count = self._error_count.get(event.source, 0)
        if count >= 5:
            insight = self._create_insight(
                title=f"Error storm on {event.source}",
                content=(
                    f"Node {event.source} had {count} errors"
                ),
                category="alert",
                severity=EventPriority.CRITICAL,
                actionable=True,
                action={"type": "investigate", "target": event.source},
            )
