...
class AliyunAdapter:
    def __init__(
        ...
        self.oss_endpoint = (
            f"https://{oss_bucket}.oss-{region}.aliyuncs.com"
        )
    ...
    async def oss_upload(self, key: str, content: str | bytes) -> bool:
        try:
            ...
            if ok:
                logger.info(f"OSS upload: {key}")
            return ok
        except Exception:
            logger.exception(f"OSS upload error: {key}")
            return False
    ...
    async def oss_download(self, key: str) -> Optional[str]:
        try:
            ...
            resp = await self._http.get(
                url, headers=self._oss_headers("GET", key)
            )
    ...
    def _oss_headers(self, method: str, key: str) -> dict[str, str]:
        ...
        auth = f"OSS {self.access_key_id}:{base64.b64encode(sig).decode()}"
        return {"Authorization": auth, "Date": now}
