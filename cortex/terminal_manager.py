...
class TerminalManager:
    ...
    async def _on_register(self, event: EventMessage):
        try:
            ...
            logger.exception("Register parse error")
    ...
    def _adjust_trust(self, node_id: NodeID, delta: float):
        ...
        self._trust_scores[node_id] = max(0.0, min(1.0, current + delta))

    def get_trust_level(self, node_id: NodeID) -> TrustLevel:
        score = self._trust_scores.get(node_id, 0.0)
        if score < 0.2:
    ...
    async def find_best_ganglion(self, task: Task) -> Optional[NodeID]:
        candidates = [
            (nid, info)
            for nid, info in self._ganglions.items()
        ...
        for nid, info in candidates:
            ...
            if task.tags and info.capabilities:
                matches = set(task.tags) & set(info.capabilities)
                cap_score = len(matches) / max(len(task.tags), 1)
    ...
    def online_ganglions(self) -> list[NodeInfo]:
        return [
            i
            for i in self._ganglions.values()
            if i.status == NodeStatus.ONLINE
        ]

    @property
    def stats(self) -> dict[str, Any]:
        return {
            "total": len(self._ganglions),
