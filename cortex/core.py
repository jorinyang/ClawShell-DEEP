...
class FeedbackControlLoop:
    def __init__(self, name: str, tolerance: float = 0.1):
        self.name = name; self.tolerance = tolerance
        self.expected: float = 0.0; self.actual: float = 0.0
        self.deviation: float = 0.0; self.iteration_count: int = 0; self.stable_count: int = 0

    def update(self, actual: float) -> float:
        self.actual = actual; self.deviation = self.expected - actual
        self.iteration_count += 1
        if abs(self.deviation) <= self.tolerance: self.stable_count += 1
        else: self.stable_count = 0
        return self._compute_signal()

    def set_target(self, target: float): self.expected = target

    def _compute_signal(self) -> float:
        kp, ki = 0.5, 0.1
        signal = kp * self.deviation + ki * self.deviation * min(self.iteration_count, 10)
        return max(-1.0, min(1.0, signal))

    @property
    def is_stable(self) -> bool: return self.stable_count >= 3
...
class StrategySwitcher:
    ...
    def evaluate(self, health_score: float, resource_pressure: float) -> Optional[Strategy]:
        new = self.current
        if health_score < 0.3: new = Strategy.EMERGENCY
        elif resource_pressure > 0.8: new = Strategy.ECONOMY
        elif health_score > 0.8 and resource_pressure < 0.3:
            if self.current in (Strategy.EMERGENCY, Strategy.ECONOMY): new = Strategy.DEFAULT
        if new != self.current and new in self.TRANSITIONS.get(self.current, []):
            return new
        return None

    def switch(self, new_strategy: Strategy, reason: str):
        old = self.current; self.current = new_strategy
...
class CortexCore:
    ...
    async def start(self, host: str = "0.0.0.0", port: int = 9000):
        ...
        self.transport = CortexTransport(node_id=self.node_id, event_bus=self.event_bus,
            host=host, port=port)
        await self.transport.start()
        self.control_loops["health"] = FeedbackControlLoop("system_health", tolerance=0.05)
    ...
    async def _on_node_offline(self, event: EventMessage):
        self.info.connected_ganglions = max(0, self.info.connected_ganglions - 1)

    async def _on_node_heartbeat(self, event: EventMessage):
        if self.transport:
            self.info.connected_ganglions = len(self.transport.connected_ganglions)
    ...
    async def _health_loop(self):
        while self.info.status != NodeStatus.OFFLINE:
            ...
            if loop: loop.update(self._health_score)
            await self._evaluate_strategy()

    async def _evaluate_strategy(self):
        new = self.strategy.evaluate(self._health_score, self._resource_pressure)
        if new:
            self.strategy.switch(new, "auto")
            event = MessageFactory.strategy_change(self.node_id, new.value)
            await self.event_bus.publish(event)
            if self.transport: await self.transport.broadcast(event)

    async def create_task(self, task: Task) -> Task:
        self._tasks[task.task_id] = task
        event = MessageFactory.task_created(task.task_id, self.node_id, task.model_dump(mode="json"))
        await self.event_bus.publish(event)
        if self.transport: await self.transport.broadcast(event)
        return task
    ...
    def stats(self) -> dict[str, Any]:
        return {"node_id": self.node_id, "uptime": self.info.uptime_seconds,
            "strategy": self.strategy.current.value, "health_score": self._health_score,
