...
class BroadcastEngine:
    ...
    async def _insight_loop(self, transport):
        while True:
            ...
            if not self._insights_buffer: continue
            ...
            self._insights_buffer = [i for i in self._insights_buffer if i not in recent]
    ...
    async def _knowledge_loop(self, transport):
        while True:
            ...
            if not self._knowledge_buffer: continue
            recent = self._knowledge_buffer[-10:]
            for k in recent:
                event = MessageFactory.knowledge_update(self.node_id, k.model_dump(mode="json"))
                if transport:
                    await transport.broadcast(event)
                    self._broadcast_count += 1
            self._knowledge_buffer = [k for k in self._knowledge_buffer if k not in recent]
            logger.info(f"Knowledge sync: {len(recent)}")

    async def generate_behavior_optimization(self, insights: list[Insight]) -> Optional[str]:
        if not insights: return None
        lines = ["# ClawShell behavior optimization",
    ...
    def stats(self) -> dict[str, Any]:
        return {"broadcast_count": self._broadcast_count,
