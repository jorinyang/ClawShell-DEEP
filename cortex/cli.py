...
@app.command()
def cortex(host: str = typer.Option("0.0.0.0", "--host", "-h"),
...
@app.command()
def ganglion(node_id: str = typer.Option("", "--id"),
...
@app.command()
def init(mode: str = typer.Option("ganglion", "--mode", "-m"),
         output: Path = typer.Option(Path("config"), "--output", "-o")):
    """Generate config files"""
    import yaml
    output.mkdir(parents=True, exist_ok=True)
    if mode == "cortex":
        cfg = {"node_id": "cortex-01", "host": "0.0.0.0", "port": 9000,
            "strategy": "default", "heartbeat_interval": 30}
        f = output / "cortex.yaml"
    else:
        cfg = {"node_id": "", "cortex_host": "localhost", "cortex_port": 9000,
            "strategy": "default", "heartbeat_interval": 30,
            "perception_interval": 60, "auto_register": True, "offline_mode": False}
