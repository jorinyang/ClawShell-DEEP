...
class PerceptionEngine:
    ...
    async def perceive_system(self) -> SystemPerception:
        try:
            ...
            logger.exception("System perception error")
            return SystemPerception()
        ...
        try:
            ...
            for p in [80, 443, 3000, 5000, 8080, 8443, 9000, 9090]:
                try:
                    ...
                    pass
            return NetworkPerception(
                hostname=hostname,
                ip_address=ip,
                open_ports=ports,
                internet_access=await self._check_internet(),
            )
        except Exception:
            logger.exception("Network perception error")
            return NetworkPerception()
    ...
    async def full_perception(
        self,
    ) -> dict[PerceptionDimension, PerceptionResult]:
        results = {}
        sd = await self.perceive_system()
        health = (
            HealthStatus.HEALTHY
            if sd.memory_percent < 90
            else HealthStatus.WARNING
        )
        results[PerceptionDimension.SYSTEM] = PerceptionResult(
            dimension=PerceptionDimension.SYSTEM,
            data=sd.model_dump(),
            health=health,
        )
        ...
        results[PerceptionDimension.NETWORK] = PerceptionResult(
            dimension=PerceptionDimension.NETWORK,
            data=nd.model_dump(),
            health=health_n,
        )
        ...
        for v, paths in sigs.items():
            for p in paths:
                if (home / p.replace("~/", "")).exists():
                    logger.info(f"Detected: {v.value}")
                    return v
        return OpenClawVariant.UNKNOWN

    @property
    def last_perception(self) -> dict:
    ...
    def system_health(self) -> HealthStatus:
        ...
        if sr:
            mp = sr.data.get("memory_percent", 0)
            cp = sr.data.get("cpu_percent", 0)
            if mp > 95 or cp > 95:
