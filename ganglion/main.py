...
async def main():
    ...
    logger.info(
        f"Cortex: {config.cortex_host}:{config.cortex_port}"
    ...
    if sd:
        logger.info(
            f"System: CPU={sd.data.get('cpu_percent', 0):.0f}%"
            f" MEM={sd.data.get('memory_percent', 0):.0f}%"
        )

    adaptation = AdaptationEngine(node_id=node_id, variant=variant)
    adapter = OpenClawAdapter(
        variant=variant, install_path=config.variant_path
    )
    arch_info = await adapter.get_architecture_info()
    agents_content = await adapter.read_agents_file()
    await adapter.install_bridge_script()
    caps = await adaptation.discover_capabilities()
    health = await adaptation.analyze_health()

    logger.info(
        f"Architecture: {arch_info.get('name', 'Unknown')}"
        f" @ {arch_info.get('config_dir', '')}"
    )
    if agents_content:
        logger.info(
            f"Agent behavior file loaded ({len(agents_content)} chars)"
        )
    logger.info(
        f"Health: {health.overall.value}"
        f" | Capabilities: {len(caps)} domains"
    )

    plugin_mgr = PluginManager(
        node_id=node_id, plugins_dir=config.plugins_dir
    )
    ...
    transport = GanglionTransport(
        node_id=node_id,
        event_bus=event_bus,
        node_info=node_info,
        cortex_host=config.cortex_host,
        cortex_port=config.cortex_port,
            ...
            if report.overall.value in ("critical", "warning"):
                for issue in report.issues:
                    ...
                    if layer.value == "self_healing":
                        ...
                        adaptation.record_healing(
                            issue.get("component", "")
                        )
                    elif layer.value == "auto_repair":
                        adaptation.record_repair(
                            issue.get("component", "")
                        )
