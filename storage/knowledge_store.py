...
class KnowledgeStore:
    ...
    async def load(self) -> int:
        ...
        for f in sorted(self.store_path.glob("*.json")):
            try:
                ...
                k = Knowledge.model_validate(data)
                self._index[k.knowledge_id] = k
                count += 1
            except Exception:
                pass
        return count
    ...
    async def store(self, knowledge: Knowledge) -> bool:
        ...
        path = self.store_path / f"{knowledge.knowledge_id}.json"
        path.write_text(
    ...
    async def search(
        ...
        for k in self._index.values():
            score = 0
            if query and query.lower() in k.title.lower():
                score += 3
            if query and query.lower() in k.content.lower():
                score += 1
            if category and k.category == category:
                score += 2
            if tags:
                score += len(set(tags) & set(k.tags))
            if score > 0:
