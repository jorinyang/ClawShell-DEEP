...
class ConfigLoader:
    ...
    def __init__(self, config_dir: Optional[Path] = None):
        self.config_dir = config_dir or Path(os.getenv("CLAWSHELL_CONFIG_DIR", "config"))
    ...
    def load_cortex(self, path: Optional[str] = None) -> CortexConfig:
        ...
        config.cloud.aliyun_ecs["access_key_id"] = os.getenv("ALIBABA_CLOUD_ACCESS_KEY_ID", config.cloud.aliyun_ecs.get("access_key_id", ""))
        config.cloud.aliyun_ecs["access_key_secret"] = os.getenv("ALIBABA_CLOUD_ACCESS_KEY_SECRET", config.cloud.aliyun_ecs.get("access_key_secret", ""))
        config.cloud.github_token = os.getenv("GITHUB_TOKEN", config.cloud.github_token)
        config.cloud.memos_cloud["api_key"] = os.getenv("MEMOS_CLOUD_API_KEY", config.cloud.memos_cloud.get("api_key", ""))
        return config
    ...
    def load_ganglion(self, path: Optional[str] = None) -> GanglionConfig:
        ...
        config.node_id = os.getenv("CLAWSHELL_GANGLION_NODE_ID", config.node_id)
        config.cortex_host = os.getenv("CLAWSHELL_CORTEX_HOST", config.cortex_host)
        port = os.getenv("CLAWSHELL_CORTEX_PORT")
        if port: config.cortex_port = int(port)
        config.offline_mode = os.getenv("CLAWSHELL_OFFLINE", "").lower() == "true"
        config.cloud.github_token = os.getenv("GITHUB_TOKEN", config.cloud.github_token)
        config.cloud.memos_cloud["api_key"] = os.getenv("MEMOS_CLOUD_API_KEY", config.cloud.memos_cloud.get("api_key", ""))
        return config
    ...
    def _merge_env(self, config: Any, data: dict[str, Any], prefix: str) -> Any:
...
def get_config_loader() -> ConfigLoader:
    ...
    if _config_loader is None: _config_loader = ConfigLoader()
    return _config_loader

def get_cortex_config() -> CortexConfig:
    global _cortex_config
    if _cortex_config is None: _cortex_config = get_config_loader().load_cortex()
    return _cortex_config

def get_ganglion_config() -> GanglionConfig:
    global _ganglion_config
    if _ganglion_config is None: _ganglion_config = get_config_loader().load_ganglion()
