...
class CortexTransport(NeuralTransport):
    ...
    async def send(self, target: NodeID, message: EventMessage):
        ...
        if conn:
            try:
                await conn.send(MessageCodec.encode_bytes(message))
            except ConnectionClosed:
                await self._remove_ganglion(target)
    ...
    async def broadcast(self, message: EventMessage):
        for node_id, conn in list(self._connections.items()):
            try:
                ...
                await self._remove_ganglion(node_id)
    ...
    async def _handle_connection(self, ws: ServerConnection):
        ...
        try:
            ...
            if (
                msg.category.value != "node"
                or msg.event_type != "node.register"
            ):
                await ws.close(1008, "Expected register")
                return
            node_id = msg.source
            ...
            async for raw in ws:
                try:
                    ...
                    logger.exception(f"Decode error from {node_id}")
    ...
    async def _remove_ganglion(self, node_id: NodeID):
        ...
        if node_id in self._ganglion_info:
            del self._ganglion_info[node_id]
        ...
        while self._running:
            ...
            for nid, info in list(self._ganglion_info.items()):
                ...
                if age > self.heartbeat_interval * 3:
                    logger.warning(
                        f"Heartbeat timeout: {nid} ({age:.0f}s)"
                    )
...
class GanglionTransport(NeuralTransport):
    ...
    async def send(self, target: NodeID, message: EventMessage):
        if self._ws and self._connected:
            try:
                await self._ws.send(
                    MessageCodec.encode_bytes(message)
                )
            except ConnectionClosed:
            ...
            try:
                ...
                async with websockets.connect(uri) as ws:
                    ...
                    while self._offline_buffer:
                        ...
                        await ws.send(MessageCodec.encode_bytes(msg))
                ...
                logger.warning(
                    f"Connect failed: {e}, retry in {retry_delay}s"
                )
                self._connected = False
                await asyncio.sleep(retry_delay)
                retry_delay = min(
                    retry_delay * 2, self.max_reconnect_interval
                )
            except Exception:
    ...
    async def _heartbeat(self, ws):
        while self._connected:
            ...
            try:
                ...
                event = MessageFactory.node_heartbeat(
                    self.node_id, hb.model_dump(mode="json")
                )
